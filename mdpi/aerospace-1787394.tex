...
\documentclass[aerospace,article,accept,moreauthors,pdftex]{Definitions/mdpi}
\usepackage{array}
\usepackage{float}
\usepackage{booktabs}
...
\pubyear{2022}
\copyrightyear{2022}
\externaleditor{\hl{Academic Editor: Firstname Lastname}} %MDPI: PLease add Academic Editor.
%Authors: We are not sure who is the Academic Editor.
\datereceived{} 
...
\Author{\hl{Weicheng Pei} $^{1}$, Yuyan Jiang $^{1}$ and Shu Li $^{1,2,}$*}
...
University, 37 Xueyuan Road, Haidian District, \hl{Beijing 100191,} \hl{China;} w\hl{eicheng.pei@icloud.com }(W.P.); ji\hl{ang.yu.yan@icloud.com }(Y.J.)\\
$^{2}$\quad{}Ningbo Institute of Technology, Beihang University,
\hl{Ningbo 315800},  \hl{China}} 

%MDPI: Please confirm removed P.R. before China, and please confirm moved  postcode after city.
%MDPI: PLease confirm added authors emails from remine.
%MDPI: we removed province in aff. 2, Please check  
%Authors: All confirmed.
...
(WENO) limiters is one of the candidates, which might give high-order
...
even on coarse meshes is shown in the results of three-dimensional
...
method; \linebreak  compact WENO limiter; parallel implementation; momentum source
models}
...
Computational fluid dynamics (CFD) solvers, widely used in aerospace engineering, are based either
...
discontinuous Galerkin (DG) finite element (FE) methods. As with other
FE methods, DG methods assume continuous approximation in each element
(i.e., cell). However, they allow discontinuities to exist on intercell
boundaries. Such discontinuities are then resolved by numerical fluxes,
e.g., exact or approximate Riemann solvers, as with Godunov-type
...
(RK) methods, which led to the family of Runge--Kutta discontinuous Galerkin (RKDG) schemes for scalar conservation
\mbox{laws \citep{Shu_1988,Shu_1989},} for~one-dimensional conservation
law systems \citep{Cockburn_1989}, for~multi-dimensional scalar conservation
\mbox{laws \citep{Cockburn_1990}} and for multi-dimensional conservation
...
scheme in Sections~\ref{subsec:spatial_discretization}~and~\ref{subsec:temporal_discretization}.

The Euler system is hyperbolic, and might have discontinuities in
its solutions. Approximating such solutions with finite terms of continuous
...
of RKDG schemes, Zhong \citep{Zhong_2013} and \mbox{Zhu \citep{Zhu_2013}}
...
We will summarize the key steps of this compact WENO limiter in
Section~\ref{subsec:limiting_procedures}.
...
bodies (such as rotary wings) have been successfully modeled as various
...
wings can be captured. Such early works are either oversimplified
...
with an unsteady actuator line model, which successfully captures unsteady
effects of a rotor on unstructured meshes. The~formulation of this
finite element/momentum source hybrid method will be given in
Section~\ref{subsec:The-Momentum-Source}. Some demonstrative results are
...
Mathematically, the~problems that we are going to solve in Section~\ref{sec:Results}
...
\begin{itemize}
\item Variables in the first column are called conservative variables;
...
except in Section \ref{subsec:Problems-with-Momentum}.
\end{itemize}
%

The method that we use for solving the Euler system (\ref{eq:euler_system}) is
...
is the flux vector whose components $\underline{F^{x}}$, $\underline{F^{y}}$,
$\underline{F^{z}}$, and $\underline{U}$, $\underline{Q}$ are all $5\times1$ matrices,
...
(i.e., cell) $E_{i}$, and~applying integral by parts and Gauss's
...
(which is the boundary of $E$).

%Authors: Add a blank line.
...
constant number. Substituting them into the weak form (Equation \ref{eq:differential_form}) gives
\begin{equation}
...
could be given by an exact or approximate Riemann solver of
Equation~(\ref{eq:differential_form}); see \citep{Toro_2009} for~details.
...
\]
and we could turn Equation~(\ref{eq:spatial_discretization}) into a system
...
in Section~\ref{subsec:temporal_discretization}), and~\begin{equation}
...
Runge--Kutta~methods:
\begin{description}
\item [\hl{first-order:}] %MDPI: Please confirm list format.
%Authors: A description list might be better.
%Authors: Commas and semicolons are added at the end of equations.
\[
\begin{aligned}\underline{\hat{U}}^{n+1} & =\underline{\hat{U}}^{n}+\underline{R}^{n}\Delta t;\end{aligned}
\]
\item [\hl{second-order:}]
\[
...
the corresponding $\underline{\hat{U}}^{\mu}$s.
...
\end{equation}
%Authors: Delete a blank line.
\textls[-15]{From the hyperbolicity of Equation~(\ref{eq:differential_form}), there
...
To model a fixed or rotary wing with a high aspect ratio, we use the
...
relative to the air, a~piece feels a pair of aerodynamic forces:
%Authors: Delete a blank line.
\[
...
in~which
\begin{itemize}
\item $c$ is the chord length of the piece, and $s$ is the arc length parameter
of the axis;
...
is the line segment $\overline{RT}$, then the body integral of the
...
These problems are usually defined as one-dimensional problems, but~we treat them as three-dimensional ones. All these problems are considered
...
\begin{Problem} %MDPI: Please confirm position is not after first citation.
%Authors: Confirmed.
[\hl{Sod}] \label{prob:sod} Solve the Euler system Equation (\ref{eq:euler_system})
for $t\in[0.0,1.0]$ with the initial~condition
%Authors: Delete a blank line.
\begin{equation*}
\begin{bmatrix}\rho & u & v & w & p\end{bmatrix}_{t=0}=\begin{cases}
\begin{bmatrix}1 & 0 & 0 & 0 & 1\end{bmatrix}, & x<2.5;\vspace{4pt}\\
...
all three types of discontinuities: a shock wave and a contact
...
\caption{\label{fig:sod_contour}\hl{Third-or}der solution of $\rho(t=1.0)$ in
...
\caption{\label{fig:sod_p}\hl{Comparison b}etween solutions of $\rho(t=1.0)$ in
Problem \ref{prob:sod}.}
\end{figure} %MDPI: Please confirm Figures moved after first citations.
%Authors: Confirmed.

It is predictable that both mesh refinement (decreasing $h$) and
order increment (increasing $p$) can help to improve accuracy. To~compare the performance of solvers with different orders more fairly,
it is better to use finer meshes for running lower-order solvers.
After a few trials, we find that the solutions given by the $h$-$p$
...
are better than low-order ones in the sense of obtaining the same level
...
\begin{tabularx}{1\textwidth}{@{\extracolsep{\fill}}>{\centering}p{0.1\textwidth}>{\centering}p{0.05\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.15\textwidth}>{\centering}p{0.1\textwidth}}
\toprule 
\boldmath{$h$} &\boldmath{ $p$} & \textbf{\#Cells} & \textbf{\#Steps} & \textbf{Time Cost} & \textbf{File Size}\tabularnewline
\midrule 
...
\begin{Problem} %MDPI: Please confirm format
%Authors: A \vspace{4pt} is added before \\.
[\hl{Lax}]\label{prob:lax}Solve the Euler system Equation (\ref{eq:euler_system})
for $t\in[0.0,0.6]$ with the initial~condition
%Authors: Delete a blank line.
\[
\begin{bmatrix}\rho & u & v & w & p\end{bmatrix}_{t=0}=\begin{cases}
\begin{bmatrix}0.445 & 0.698 & 0.000 & 0.000 & 3.528\end{bmatrix}, & x<2.5;\vspace{4pt}\\
...
This is another problem that involves all three types of discontinuities.
...
\vspace{-6pt}
\begin{figure}[H]
\hspace{-18pt} %MDPI: Please confirm figures moved after first citation.
%Authors: Confirmed.
\includegraphics[width=1\textwidth]{figures/shock_tubes/lax/contour_tetra}

\caption{\label{fig:lax_contour}\hl{Third-order} solution of $\rho(t=0.5)$ in
...
\caption{\label{fig:lax}Co\hl{mparison b}etween solutions of $\rho(t=0.5)$ in
Problem \ref{prob:lax}.}
\end{figure}

\begin{Problem}
[\hl{Vacuum}]\label{prob:vacuum}\textls[-15]{Solve the Euler system Equation (\ref{eq:euler_system})
for $t\in[0.0,0.4]$ with the initial~condition}
%Authors: Delete a blank line.
%Authors: A \vspace{4pt} is added before \\.
\[
\begin{bmatrix}\rho & u & v & w & p\end{bmatrix}_{t=0}=\begin{cases}
\begin{bmatrix}1 & -4 & 0 & 0 & 0.4\end{bmatrix}, & x<2.5;\vspace{4pt}\\
...
strong discontinuities (shock or contact discontinuity) occur in
the solution. However, there is a region of vacuum, which does not
occur in the previous problems, generated between the left- and right-running
expansion waves. The~velocity $u$ inside the vacuumed region (denoted
by a $*$ in subscript) is undefined, but~its boundary values should
be determined from the left (denoted by an $L$ in subscript) and
right (denoted by an $R$ in subscript) initial values by the two
Riemann invariants:
%Authors: Delete a blank line.
\[
u_{*L}=u_{L}+\frac{2a_{L}}{\gamma-1},\qquad u_{*R}=u_{R}-\frac{2a_{R}}{\gamma-1}.
\]
See \citep{Toro_2009} for more detailed discussions. %MDPI: Please check and revise Section 4.6 does not exist in the document.
%Authors: Removed.
Unfortunately, approximate Riemann solvers usually (have to) neglect
...
Figures~\ref{fig:vacuum_contour} and~\ref{fig:vacuum}, which
again validate the correctness and robustness of our~solvers.

\begin{figure}[H]
\hspace{-13pt}
\includegraphics[width=0.9\textwidth]{figures/shock_tubes/vacuum/contour_tetra}

\caption{\label{fig:vacuum_contour}\hl{Third-orde}r solution of $\rho(t=0.4)$
...
\begin{figure}[H]
\hspace{-13pt}
\includegraphics[width=0.9\textwidth]{figures/shock_tubes/vacuum/result_tetra}

\caption{\label{fig:vacuum}C\hl{omparison }between solutions of $\rho(t=0.4)$
in Problem \ref{prob:vacuum}.}
\end{figure}
...
\textls[-25]{This is a classical two-dimensional problem \citep{Woodward_1984},
...
solve this problem on meshes such as the one in Figure~\ref{fig:forward_step_domain}.
...
\caption{\label{fig:forward_step_domain}\hl{A coarse} ($h=1/20$) mesh for solving
...
\caption{\label{fig:forward_step_p=00003D3_t=00003D02e-1}\hl{Third-order} %MDPI: figures 9-19: Is the space above the image necessary? If not, crop it for a better display 
%Authors: OK. Trimed figures are used.
 solution
...
The flow starts from a uniform supersonic ($M=3.0$) state. In~Figures~\ref{fig:forward_step_p=00003D3_t=00003D02e-1} and \ref{fig:forward_step_p=00003D3_t=00003D04e-1},
...
and \ref{fig:forward_step_p=00003D3_t=00003D10e-1}). At~approximately $t=1.2$
...
Shortly before $t=2.0$ (Figure \ref{fig:forward_step_p=00003D3_t=00003D12e-1}),
a Mach stem that characterizes a Mach reflection is generated from
...
in the surroundings of a YF-17 aircraft. The~initial condition is given
...
\]
%Authors: Delete a blank line.
A\hl{ll boundaries of the aircraft are defined as solid walls, except~the intake of the engine, which is defined as a subsonic outlet of the
condition equal to the initial condition, and~the exhaust of the engine,
which is defined as a sup}ersonic ($M=3.0$) inlet of the condition: %MDPI: Please confirm italics format.
%Authors: Confirmed. They are within the problem environment.
%Authors: Delete a blank line.
\[
...
We solve this problem via two of our finite element solvers and plot
the streamlines released from the strake and the wing at $t=10$ in
Figures~\ref{fig:yf17_streamlines_p=00003D1} and \ref{fig:yf17_streamlines_p=00003D3}.
...
backward, and the~tip vortex is thus formed. The~vortex trailing from
the strake is generated from a similar~mechanism.


\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/streamlines_p=1}

\caption{\label{fig:yf17_streamlines_p=00003D1}S\hl{treamlines }given by the first-order
solution of Problem \ref{prob:Subsonic-YF17}.}
\end{figure}
\unskip

\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/streamlines_p=3}

\caption{\label{fig:yf17_streamlines_p=00003D3}\hl{Streamline}s given by the third-order
solution of Problem \ref{prob:Subsonic-YF17}.}
\end{figure}

\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/rho_subsonic}
...
\]
%Authors: Delete a blank line.
All bo\hl{undaries of the aircraft are defined as solid walls, except~the intake of the engine, which is defined as a supersonic outlet of the
condition equal to the initial conditio}n, and~the exhaust of the engine,
...
the third-order solutions are given in Figures~\ref{fig:yf17_rho_p=00003D1}
and~\ref{fig:yf17_rho_p=00003D3}, respectively. It is obvious
...
two solutions in Figures~\ref{fig:yf17_supersonic_top_view} and \ref{fig:yf17_supersonic_bottom_view}. As~in the subsonic case, the~third-order solution (which is piecewise quadratic) outperforms the
...
which leaves more room for the supersonic flow.


\begin{figure}[H] %MDPI: Please check and revise Fiures 24 and 26 are look the same.
%Authors: Do you mean Figures 24 and 25? If so, it is confirmed that they are different.
%MDPI: Please confirm figures moved after first citations.
%Authors: Confirmed.
\includegraphics[width=0.9\textwidth]{figures/yf17/rho_p=1} %MDPI: Please confirm reducing the size of the figure.
%Authors: Confirmed.

\caption{\label{fig:yf17_rho_p=00003D1}\hl{Density contour obtained from the first-orde}r
solution of Problem \ref{prob:Supersonic-YF17}.}
\end{figure}
\unskip

\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/rho_p=3}

\caption{\label{fig:yf17_rho_p=00003D3}D\hl{ensity contour obtained from the third-order}
solution of Problem \ref{prob:Supersonic-YF17}.}
\end{figure}
\unskip

\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/supersonic_top_view}

\caption{\label{fig:yf17_supersonic_top_view}Com\hl{parison between} the third-order
...
\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{figures/yf17/supersonic_bottom_view}

\caption{\label{fig:yf17_supersonic_bottom_view}Co\hl{mparison between} the third-order
solution (upper half) and the first-order solution (lower half) of
...
balanced distribution of computational effort. In~our code, we use
...
\begin{figure}[H] %MDPI: Figures moved after first citation, please confirm. %MDPI: Please confirm reducing the size of the figure.
%Authors: Both confirmed.
\includegraphics[width=0.9\textwidth]{figures/yf17/partition}

\caption{\label{fig:yf17_partition}A \hl{100-part partition}ing of the mesh used
...
\begin{figure}[H]
\hspace{-10pt}
\includegraphics[width=0.9\textwidth]{figures/yf17/balance}

\caption{\label{fig:yf17_balance}Distri\hl{bution of c}ells in the mesh partitioning
given in Figure~\ref{fig:yf17_partition}.}
\end{figure}

\begin{figure}[H]
\hspace{-10pt}
\includegraphics[width=0.8\textwidth]{figures/yf17/efficiency}%MDPI: Please confirm reducing the size of the figure.
%Authors: Confirmed.

\caption{\label{fig:yf17_efficiency}P\hl{arallel} efficiency of the third-order
...
\begin{tabularx}{1\textwidth}{@{\extracolsep{\fill}}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}>{\centering}p{0.1\textwidth}}
\toprule 
\boldmath{$P$} & \boldmath{$T_{100}$} & \boldmath{$T_{399}$} & \boldmath{$T_{400}$} & \boldmath$P\frac{T_{399}-T_{100}}{399-100}$ &\boldmath$P\frac{T_{400}-T_{100}}{400-100}$\tabularnewline
\midrule 
1 & \hl{10,511}.3 & \hl{42,466}.5 & \hl{42,593}.0 & 106.873 & 106.939\tabularnewline
...
\bottomrule
\end{tabularx}
\end{table} %MDPI: Please confirm added comma on five-digits numbers.
%Authors: Confirmed.
%\unskip
...
left and right ends, which are set as the inlet and outlet, respectively.
The spherical region is the circumscribed sphere of the rotor, whose
rotating axis could point in any direction. The~smaller box enclosing
the sphere is used for refining the mesh in the surrounding and downstream
regions of the~rotor.

\begin{figure}[H]
\hspace{-28pt}
\includegraphics[width=0.8\textwidth]{figures/rotor_in_tunnel/mesh}

\caption{\label{fig:rotor_mesh}A schematic mesh for solving problems in
Section~\ref{subsec:Problems-with-Momentum}.}
\end{figure}
\unskip
...
in the surroundings of a rotor, whose rotating axis points in the direction
...
\textls[-35]{Third- and first-order solutions at the same moment ($t=1$) are shown
in Figures~\ref{fig:rotor_upward_p=00003D3} and~\ref{fig:rotor_upward_p=00003D1},
...
contraction of airflow near the rotor disk and the rolled-up airwake,
...
the downstream, are only visible in the third-order solution. The~nearly axisymmetric flow structure is caused by the periodic movement
of the rotary wings. The~density of air immediately below the rotor (red)
is greater than that above the rotor (blue), which means that the rotor
...
\caption{\label{fig:rotor_upward_p=00003D3}\hl{Third-ord}er solution of Problem
...
\caption{\label{fig:rotor_upward_p=00003D1}\hl{First}-order solution of Problem
\ref{prob:rotor_upward} at $t=1$.}
\end{figure}
...
in the surrounding of a rotor, whose rotating axis points in the direction
...
As with the climbing problem, we plot the third- and first-order
solution at the same moment ($t=1$) in Figures~\ref{fig:rotor_forward_p=00003D3}
and \ref{fig:rotor_forward_p=00003D1}, respectively. As~before,
both of them can capture large-scaled structures, such as the skewed
and rolled-up airwakes. The~rolled-up structure is more clear in
Figure~\ref{fig:rotor_forward_wake}, in~which streamlines are plotted
and colored by the magnitude of velocity. The~piecewise constant
first-order solution is much vaguer and loses many details, which
...
\caption{\label{fig:rotor_forward_p=00003D3}T\hl{hird}-order solution of Problem
...
\caption{\label{fig:rotor_forward_p=00003D1}F\hl{irst}-order solution of Problem
\ref{prob:rotor_forward} at $t=1$.}
\end{figure}
\unskip
...
\caption{\label{fig:rotor_forward_wake}S\hl{treamline}s of the solution in
Figure~\ref{fig:rotor_forward_p=00003D3}.}
\end{figure}
...
implementing higher-order solvers under this framework and testing
...
\dataavailability{The data presented in this article are all generated from the source %MDPI: Please add accessed on date on links.
%Authors: Added.
code publicly available in our GitHub \hl{repository at} \url{https://github.com/pvc1989/miniCFD} accessed on 10 July 2022 (or the mirror \hl{site} \url{https://gitee.com/pvc1989/miniCFD} accessed on 10 July 2022), except~the mesh used in Section~\ref{subsec:YF-17}, which was downloaded \hl{from}
\url{https://cgns.github.io/CGNSFiles.html}  accessed on 13 May 2022.}

\acknowledgments{\textls[-5]{The authors would like to express their deepest appreciation to Shen
Xin for his professional guidance on computer science, and~to Zhou
Yukai for his patient assistance with~mathtyping.}}

\conflictsofinterest{The authors declare no conflicts of interest. The~funders had no role
...
\abbreviations{Abbreviations}{ %MDPI: Please check can it be merge Abbreviations and Nomenclature to one list.  % PVC: Yes.
\hl{The following abbreviations} are used in~this manuscript:\\
...
\hl{The following symbols are used in this ma}nuscript:\\
...
\noindent {\fontsize{9}{11.2}\selectfont
\begin{tabular}{@{}ll}

$\underline{\vec{F}}$ & flux vector whose components are matrices\tabularnewline
...
$\underline{\hat{U}}^{n}$ and $\underline{R}^{n}$ & $\underline{\hat{U}}$ and $\underline{R}$ at the $n$-th time step\tabularnewline
...
\bibitem[Harten \em{et~al.}(1983)Harten, Lax, and van Leer]{Harten_1983}
Harten, A.; Lax, P.D.; van Leer, B.
\newblock On Upstream Differencing and Godunov-Type Schemes for Hyperbolic
  Conservation Laws.
\newblock {\em {SIAM} Rev.} {\bf 1983}, {\em 25},~35--61, https://doi.org/10.1137/1025002.

\bibitem[Reed and Hill(1973)]{Reed_1973}
Reed, W.H.; Hill, T.R.
\newblock {\em Triangular Mesh Methods for the Neutron Transport Equation} %MDPI: Please confirm added information.
%Authors: Confirmed. Thanks for the completion.
\newblock Technical Report LA-UR-73-479; Los Alamos Scientific Lab.: \hl{Los Alamos, MN, USA,}  1973.
...
\newblock {\em J. Comput. Phys.} {\bf 1982}, {\em
  45},~307--344, https://doi.org/10.1016/0021-9991(82)90107-3.
...
\newblock  In Proceedings of the  1st National Fluid Dynamics Conference, Cincinnati, OH, USA, 25--28
  July 1988; American Institute of Aeronautics and Astronautics: Reston, VA,
  USA,  1988, https://doi.org/10.2514/6.1988-3797.
...
\newblock {\em J. Comput. Phys.} {\bf 1988}, {\em
  77},~439--471, https://doi.org/10.1016/0021-9991(88)90177-5.

\bibitem[Shu and Osher(1989)]{Shu_1989}
Shu, C.W.; Osher, S.
\newblock Efficient implementation of essentially non-oscillatory
  shock-capturing schemes, {II}.
\newblock {\em J. Comput. Phys.} {\bf 1989}, {\em 83},~32--78, https://doi.org/10.1016/0021-9991(89)90222-2.
...
\newblock {\em J. Comput. Phys.} {\bf 1989}, {\em 84},~90--113, https://doi.org/10.1016/0021-9991(89)90183-6.
...
\newblock {\em Math. Comput.} {\bf 1990}, {\em 54},~545--581, https://doi.org/10.1090/s0025-5718-1990-1010597-0.

\bibitem[Cockburn and Shu(1998)]{Cockburn_1998a}
Cockburn, B.; Shu, C.W.
\newblock The Runge{\textendash}Kutta Discontinuous Galerkin Method for
  Conservation Laws V: Multidimensional Systems.
\newblock {\em J. Comput. Phys.} {\bf 1998}, {\em
  141},~199--224, https://doi.org/10.1006/jcph.1998.5892.

\bibitem[Gottlieb and Shu(1997)]{Gottlieb_1997}
{Gottlieb, D.; Shu, C.W.
\newblock On the Gibbs Phenomenon and Its Resolution.
\newblock {\em SIAM Rev.} {\bf 1997}, {\em 39},~644–668, \linebreak https://doi.org/10.1137/S0036144596301390.}

\bibitem[Sweby(1984)]{Sweby_1984}
Sweby, P.K.
\newblock High Resolution Schemes Using Flux Limiters for Hyperbolic
  Conservation Laws.
\newblock {\em {SIAM} J. Numer. Anal.} {\bf 1984}, {\em
  21},~995--1011, https://doi.org/10.1137/0721062.
...
\newblock Uniformly High Order Accurate Essentially Non-oscillatory Schemes,
  {III}.
\newblock {\em J. Comput. Phys.} {\bf 1997}, {\em 131},~3--47, https://doi.org/10.1006/jcph.1996.5632.
...
\newblock {\em J. Comput. Phys.} {\bf 1996}, {\em
  126},~202--228, https://doi.org/10.1006/jcph.1996.0130.
...
\newblock {\em J. Comput. Phys.} {\bf 1999}, {\em
  150},~97--127, https://doi.org/10.1006/jcph.1998.6165.
...
\newblock {\em {SIAM} J. Sci. Comput.} {\bf 2005}, {\em
  26},~907--929, https://doi.org/10.1137/s1064827503425298.
...
\newblock {\em J. Comput. Phys.} {\bf 2008}, {\em
  227},~4330--4353, https://doi.org/10.1016/j.jcp.2007.12.024.
...
\newblock {\em J. Comput. Phys.} {\bf 2013}, {\em
  232},~397--415, https://doi.org/10.1016/j.jcp.2012.08.028.
...
\newblock {\em J. Comput. Phys.} {\bf 2013}, {\em
  248},~200--220, https://doi.org/10.1016/j.jcp.2013.04.012.

\bibitem[Pei \em{et~al.}(2022)Pei, Jiang, and Li]{Pei_2022}
Pei, W.; Jiang, Y.; Li, S.
\newblock An Efficient Parallel Implementation of the Runge{\textendash}Kutta
  Discontinuous Galerkin Method with Weighted Essentially Non-Oscillatory
  Limiters on Three-Dimensional Unstructured Meshes.
\newblock {\em Appl. Sci.} {\bf 2022}, {\em 12},~4228, https://doi.org/10.3390/app12094228.

\bibitem[Rajagopalan and Mathur(1989)]{Rajagopalan_1989}
Rajagopalan, R.G.; Mathur, S.R.
\newblock Three dimensional analysis of a rotor in forward flight.
\newblock  In Proceedings of the 20th Fluid Dynamics, Plasma Dynamics and Lasers Conference, \hl{Buffalo, NY, USA,  12--14 J}uly 1989; American
  Institute of Aeronautics and Astronautics: \hl{Reston, VA, USA, } 1989, https://doi.org/10.2514/6.1989-1815.

\bibitem[Rajagopalan and Mathur(1993)]{Rajagopalan_1993}
Rajagopalan, R.G.; Mathur, S.R.
\newblock Three Dimensional Analysis of a Rotor in Forward Flight.
\newblock {\em J. Am. Helicopter Soc.} {\bf 1993}, {\em
  38},~14--25, https://doi.org/10.4050/JAHS.38.14.

\bibitem[Kang and Sun(1997)]{Kang_1997}
Kang, N.; Sun, M.
\newblock Technical Note: Prediction of the Flow Field of a Rotor in Ground
  Effect.
\newblock {\em J. Am. Helicopter Soc.} {\bf 1997}, {\em
  42},~195--198, https://doi.org/10.4050/jahs.42.195.

\bibitem[Kang and Sun(2000)]{Kang_2000}
Kang, N.; Sun, M.
\newblock Simulated Flowfields in Near-Ground Operation of Single- and
  Twin-Rotor Configurations.
\newblock {\em J. Aircr.} {\bf 2000}, {\em 37},~214--220, https://doi.org/10.2514/2.2609.

\bibitem[Shi \em{et~al.}(2017)Shi, Xu, Zong, and Xu]{Shi_2017}
Shi, Y.; Xu, Y.; Zong, K.; Xu, G.
\newblock An investigation of coupling ship/rotor flowfield using steady and
  unsteady rotor methods.
\newblock {\em Eng. Appl. Comput. Fluid Mech.} {\bf
  2017}, {\em 11},~417--434, https://doi.org/10.1080/19942060.2017.1308272.

\bibitem[Shen \em{et~al.}(2009)Shen, Zhang, and S{\o}rensen]{Shen_2009}
Shen, W.Z.; Zhang, J.H.; S{\o}rensen, J.N.
\newblock The Actuator Surface Model: A New Navier{\textendash}Stokes Based
  Model for Rotor Computations.
\newblock {\em J. Sol. Energy Eng.} {\bf 2009}, {\em 131}, https://doi.org/10.1115/1.3027502.

\bibitem[Kim \em{et~al.}(2015)Kim, Oh, and Yee]{Kim_2015}
Kim, T.; Oh, S.; Yee, K.
\newblock Improved actuator surface method for wind turbine application.
\newblock {\em Renew. Energy} {\bf 2015}, {\em 76},~16--26, https://doi.org/10.1016/j.renene.2014.11.002.

\bibitem[Toro(2009)]{Toro_2009}
Toro, E.F.
\newblock {\em Riemann Solvers and Numerical Methods for Fluid Dynamics};
  Springer: B\hl{erlin/Heidelberg, Germany,  }2009, https://doi.org/10.1007/b79761.
...
\newblock {\em Math. Comput.} {\bf 1998}, {\em 67},~73--85, \linebreak https://doi.org/10.1090/s0025-5718-98-00913-2.

\bibitem[Gottlieb \em{et~al.}(2001)Gottlieb, Shu, and Tadmor]{Gottlieb_2001}
Gottlieb, S.; Shu, C.W.; Tadmor, E.
\newblock Strong Stability-Preserving High-Order Time Discretization Methods.
\newblock {\em {SIAM} Rev.} {\bf 2001}, {\em 43},~89--112, https://doi.org/10.1137/s003614450036757x.
...
\newblock {\em J. Comput. Phys.} {\bf 1984}, {\em
  54},~115--173, https://doi.org/10.1016/0021-9991(84)90142-6.
...
\newblock {\em {SIAM} J. Sci. Comput.} {\bf 1998}, {\em
  20},~359--392, https://doi.org/10.1137/s1064827595287997.
